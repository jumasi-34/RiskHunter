...
### ② 상단 위 고정 및 프리미엄 다크 슬레이트 블루 대시보드 규격 (Reverse-Sync)
사용자의 스크롤 시 편의성을 극대화하기 위해 상단 필터 바, 활성 일정 선택기 및 실시간 대시보드를 통째로 위 고정(Sticky Header)으로 묶고, 대시보드는 슬레이트 다크 테마 기반의 프리미엄 보드로 고정 설계합니다.
*   **Sticky Wrapper (.sticky-planning-header)**:
    - position: sticky, z-index: 100을 통해 서브탭 내용 스크롤 시 상단에 견고히 밀착 고정.
*   **다크 슬레이트 블루 대시보드 (.monitoring-dashboard-card)**:
    - 백그라운드 `#0f172a`, 테두리 `rgba(255, 255, 255, 0.12)`, 텍스트 `#f8fafc`를 통해 강렬하고 지적인 하이엔드 테크 감성 및 WCAG 2.1 AA 명도 대비(4.5:1 이상) 충족.
    - 2x2 디테일 정보는 반투명 어두운 슬레이트 레이어인 `rgba(255, 255, 255, 0.06)` 배경 적용.
    - 5대 가로 KPI 카드들(.kpi-sub-card)은 `rgba(255, 255, 255, 0.05)` 배경 및 마우스 호버(Hover) 시 부드러운 상향 슬라이딩 입체 애니메이션 구현.
