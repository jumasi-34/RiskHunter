...
전체 화면은 "단정하고 신뢰성 넘치는 프리미엄 B2B 엔터프라이즈 콘솔 (Premium Clean B2B Enterprise Console)" 테마(맑은 라이트 캔버스 `--bg-app: #f8fafc` 및 순백색 솔리드 표면 `--bg-card: #ffffff`)에 기반하며, 포인트 로열 블루(`--brand-blue: #2563eb`) 액센트 컬러를 반영하여 계기판 감성의 하이엔드 레이아웃으로 설계합니다.
