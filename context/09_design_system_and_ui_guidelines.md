...
*   **구성**: 신규 감사 일정 등록 모달 등, 입력 및 설정 폼이 결합된 중요 팝업 패널.
*   **스타일링**: 복잡한 반투명 글래스 뒤에 텍스트나 입력란이 묻히는 가독성 훼손을 원천 차단하기 위해 완전한 불투명 솔리드 화이트 백그라운드(`#ffffff`)와 명확한 그레이 보더(`#cbd5e1`), 그리고 차콜 네이비 폰트(`#0f172a`)를 강제 매핑하여 WCAG 2.1 AA 규격(4.5:1 이상 대비율)을 빈틈없이 만족합니다.
...
*   **디자인 계약**:
    ...
    *   **테이블 반응형 샌드박스**: 모든 대형 그리드 및 데이터 테이블은 반드시 `class="table-responsive"` 래퍼 안에 감싸고, 해당 래퍼에 `overflow-x: auto; overflow-y: auto; min-width: 0; width: 100%;` 규칙을 기입해 카드를 뚫고 나가지 않고 내부 스크롤로 매끄럽게 흡수되게 완결합니다.
