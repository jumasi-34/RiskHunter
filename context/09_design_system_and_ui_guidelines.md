...
### ⑧ Chart / Gauge (분석 비율 도넛 차트)
*   **구성**: 분석 비율을 표현하는 도넛 차트 (기존 수검 전체 준비율 원형 게이지는 직관성 향상을 위해 제거됨).
*   **스타일링**: 자체 SVG 드로잉을 기반으로 도넛 루프와 범례 텍스트를 렌더링합니다. 차트 아크의 바탕 트랙은 세련된 브랜드 전용 팔레트로 처리하여 프리미엄 계기판 감성을 구현합니다.
...
### ⑥ Audit Timeline & Real-time Monitoring Dashboard (감사 일정 관리 타임라인 및 실시간 계기판)
...
*   **실시간 감사 준비 현황 모니터링 계기판 (.monitoring-dashboard-card)**:
    *   **상단 타이틀 영역**: `.monitoring-header` 및 `.monitoring-status-badge`로 구성된 가로 정렬 바.
    *   **중단 5대 KPI 그리드 (.monitoring-kpi-grid)**: 5대 요약 KPI 카드(`.kpi-sub-card`)가 화면 전체 너비를 채우는 그리드입니다. 미디어 쿼리를 내장하여 데스크탑/노트북(1100px 초과)에서는 `repeat(5, 1fr)`로 정렬되고, 소형 노트북/태블릿(1100px 이하)에서는 `repeat(3, 1fr)`, 모바일(768px 이하)에서는 `repeat(2, 1fr)`로 유연히 축소 및 재배열되어 카드 및 폰트 찌그러짐을 완벽 방지합니다.
    *   **하단 준비율 요약 영역 (.monitoring-gauge-area)**: 준비율 텍스트 영역을 감싸며 `flex-shrink: 0; min-width: max-content;` 속성으로 어떤 뷰포트 크기 변화에도 텍스트와 준비율 수치가 깨지지 않도록 절대 레이아웃 보호를 장착합니다. (기존 원형 게이지는 직관성을 위해 제거됨)
    *   **하단 4대 디테일 칩 (.monitoring-detail-grid)**: 4분할 디테일 칩들이 가로로 반응형 배치됩니다. 가로 너비 부족 시(1200px 이하) 하단에 완전히 정렬되고, 850px 이하에서는 `repeat(2, 1fr)` (2열 2행), 480px 이하에서는 1열 형태로 컴팩트하게 재배치되어 텍스트와 레이아웃이 뭉개지지 않고 우아하게 보존됩니다.
*   **5단계 요약 KPI 카드 (Metric Indicator Array)**:
    ...
    *   글자 및 패딩 조절: `.kpi-sub-card`에는 `padding: 8px 6px` 및 `box-sizing: border-box`를 의무 적용하고, 작은 화면(max-width: 1400px)에서는 수치 폰트 크기를 `13.5px`로 소형화하여 겹침을 방지합니다.
*   **마일스톤 타임라인 & 태스크 그리드 (Milestone & Grouped Tasks Panel)**:
    *   **좌측 수직 타임라인**: 수직 실선과 둥근 로케이션 노드 제공. `D-30 ~ D-21` (준비 계획 수립), `D-20 ~ D-8` (실행 및 증빙 확보 - 포커스 음영/테두리), `D-7 ~ D-1` (최종 점검 및 시뮬레이션), `D-Day (감사)` (레드 노드). 마일스톤 카드(`.milestone-timeline-card`)에는 충분한 인너 마진과 시각 구조 확보를 위해 `padding: 14px 16px` 및 `border: 1.5px solid var(--border-card)`를 선언했으며, 활성 현단계 마일스톤 카드(`.active-glow`)에는 브랜드 블루 테두리(`border-color: var(--brand-blue)`), 은은한 로열 블루 미세기 배경 틴트(`background: rgba(37, 99, 235, 0.025)`), 그리고 프리미엄 소프트 쉐도우(`box-shadow: 0 8px 24px rgba(37, 99, 235, 0.12)`)를 병합 투사하여 가시적 위계와 개방감을 최상위로 연계 증강했습니다.
